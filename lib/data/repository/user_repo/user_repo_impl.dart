...
class UserRepoImpl extends UserRepo{
  ...
  Stream<UserModelRepo> getMyProfileStream() async* {
    ...
    yield* firebaseUserAPi.getMyProfileStream(userId ?? '').map((event) => UserModelRepo.fromFirebase(event));
  }

  @override
  Future<UserModelRepo> getUser(String userId) {
    return firebaseUserAPi.getUser(userId).then((value) => UserModelRepo.fromFirebase(value));
  }
}